...
- **Expensive day — switch on early or wait?** Choose between switching on up to an hour early (default 21:00) and waiting for the trigger (22:00). It defaults to the **earliest** early slot (most cool-down) but shifts to a later early slot (e.g. 21:30) only if that's **quite a bit cheaper** (≥ the margin, default 8 p/kWh) — so a trivial 30-min gap won't land you on an awkward in-between time. It then goes early only if the resulting run is **not too pricey** (under your limit, default 32 p/kWh) **and within the margin of the trigger price**; otherwise it says wait. The card lists the individual 21:00 / 21:30 / 22:00 prices (band-coloured) so you can see the comparison.
